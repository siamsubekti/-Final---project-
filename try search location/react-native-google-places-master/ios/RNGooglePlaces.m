...
RCT_EXPORT_METHOD(lookUpPhotosForPlaceID: (NSString*)placeID
                  resolver: (RCTPromiseResolveBlock) resolve
                  rejecter: (RCTPromiseRejectBlock) reject)
{
    NSMutableArray *googlePlacePhotoList = [NSMutableArray array];
    [[GMSPlacesClient sharedClient] lookUpPhotosForPlaceID:placeID callback:^(GMSPlacePhotoMetadataList *results, NSError *error) {
        if (error != nil) {
            reject(@"E_PHOTOMETA_ERROR", [error description], nil);
            return;
        }
        __block int photoCount = (int)[results.results count];
        for(GMSPlacePhotoMetadata* result in results.results) {
            __block NSMutableDictionary *photoData = [[NSMutableDictionary alloc] init];
            
            [[GMSPlacesClient sharedClient] loadPlacePhoto:result callback:^(UIImage *_Nullable photo, NSError *_Nullable error) {
                NSMutableDictionary *photoSize = [[NSMutableDictionary alloc] init];
                photoSize[@"width"] = [NSNumber numberWithFloat:result.maxSize.width];
                photoSize[@"height"] = [NSNumber numberWithFloat:result.maxSize.height];
                photoData[@"size"] = photoSize;
                if (error != nil) {
                    photoData[@"url"] = @"";
                } else {
                    NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
                    NSString *filePath = ([paths count] > 0) ? [paths objectAtIndex:0]: nil;
                    NSString *extension = @".jpg";
                    NSString *tempPath = [NSString stringWithFormat:@"%@/%@%@", filePath,result.attributions.string, extension];
                    NSLog(@"%@", tempPath);
                    [UIImagePNGRepresentation(photo) writeToFile:tempPath atomically:YES];
                    NSLog(@"Getting Image success");
                    photoData[@"url"] = tempPath;
                }
                [googlePlacePhotoList addObject:photoData];
                photoCount = photoCount -1;
                if ( photoCount == 0) {
                    resolve(googlePlacePhotoList);
                }
            }];
            
            photoData[@"attributions"] = result.attributions.string;
            NSLog(@"==============");
        }
